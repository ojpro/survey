  ...
  return (
    <div className='bg-white shadow p-4 rounded-md my-3'>
      ...
      <div>
        ...
        {question.type === 'text' && (
          <div className='my-2'>
            <input type="text" onChange={(event) => onAnswerChange(event.target.value)} className='rounded border-gray-300 shadow-sm focus:ring-blue-500'/>
          </div>
        )}
        {question.type === 'select' && (
          <div className='my-2'>
            <select name="" id="" onChange={(event) => onAnswerChange(event.target.value)} className='rounded border-gray-300 shadow-sm focus:ring-blue-500'>
        ...
        {question.type === 'checkbox' && (
          <div className='my-2'>
            {question.data.options.map((option, index) => (
              <div key={option.uuid}>
                <input id={option.uuid} name={'question-' + option.id} type='checkbox' value={option.uuid} className='mr-2'
        ...
        {question.type === 'textarea' && (
          <div className='my-2'>
            <textarea onChange={(event) => onAnswerChange(event.target.value)} className='rounded border-gray-300 shadow-sm focus:ring-blue-500 w-full md:w-1/2 lg:w-1/3'></textarea>
          </div>
        )}
      </div>
    </div>
