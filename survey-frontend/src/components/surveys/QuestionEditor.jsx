...
export default function QuestionEditor({index = 0, question, deleteQuestion, questionChange, addQuestion}) {
  ...
  // handle pressing [enter] key in keyboard
  const handleKeyboardPressOnInput = (event, index) => {
  ...
  return (<>
    <div className='border border-gray-200 bg-white shadow-sm p-4 rounded-md flex flex-col'>
      ...
      <div className='flex gap-3 justify-between mb-3'>
        ...
        <div>
          ...
          <select name="question_type" id="question_type" value={model.type}
            ...
            {questionTypes.map((type) => (<option value={type} key={type}>{upperCaseFirst(type)}</option>))}
          </select>
        </div>
        {/* // Question Type */}
      </div>
      ...
      {/* Question Options */}
      {shouldHaveOptions() && (<div className='my-2 p-2'>
        ...
        {(!model.data.options || model.data.options.length === 0) && (<div className='text-gray-800 text-center my-4'>
          Question doesn't have any option.
        </div>)}

        {model.data.options && model.data.options.length > 0 && (<div className='flex flex-col gap-2'>
          {model.data.options.map((option, index) => (<div key={index}>
